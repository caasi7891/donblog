...
export async function GET(req: NextRequest) {
  ...
  const baseCode = normalizeStockCode(stockCode);

  try {
    // Phase 1: Check local file cache
    const cached = await getCachedCandles(baseCode, date);
    if (cached) {
    ...
    const codesToTry = [`${baseCode}_AL`, baseCode, `A${baseCode}`].filter(
      (v, i, a) => a.indexOf(v) === i
    );

    for (const config of accounts) {
      console.log(
        `[DEBUG] Fetching live 1m chart for ${baseCode} on ${date} using ${config.label}`
      );
    ...
    // Phase 3: Archive to local file cache
    if (candles.length > 0) {
      console.log(
        `[DEBUG] Archiving chart for ${baseCode} on ${date} (${candles.length} candles)`
      );
      try {
        await setCachedCandles(baseCode, date, candles);
      } catch (archiveError) {
